...
public class PuzzleStarters {
    ...
    public static void main(String[] args) {
         ...
         * Queue<Character> directions = new LinkedList(); char[] map = { 'L', 'R', 'L',
         * 'L', 'R', 'R' }; System.out.println("Find your way out!!"); while (trapped) {
         * // fun hallway statement
         * System.out.println("Press L for left or R for right"); // input L or R for
         * direction // later implementation in application, use a L button or a R
         * button String input = sc.nextLine();
         * 
         * char dir = input.charAt(0); if (dir == 'l' || dir == 'L') {
         * directions.offer('L');
         * 
         * } else if (dir == 'r' || dir == 'R') { directions.offer('R');
         * 
         * } else if (input.equalsIgnoreCase("hint")) { hintCnt = giveHint("Recursion",
         * hintCnt); System.out.println("Are you dumb?"); } else {
         * System.out.println("Not an option my friend"); } if (directions.size() ==
         * map.length) { for (int i = 0; i < map.length; i++) { if (directions.peek() ==
         * map[i]) { directions.poll(); if (directions.isEmpty()) { trapped = false; } }
         * else { System.out.println("You took a wrong turn!! Go back!"); // start at
         * beginning directions.clear(); trapped = true; break; // put back at beginning
